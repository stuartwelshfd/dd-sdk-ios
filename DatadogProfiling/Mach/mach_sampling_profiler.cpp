...
bool mach_sampling_profiler::start_sampling() {
    ...
    if (running.load(std::memory_order_relaxed)
        || sampling_thread_started.load(std::memory_order_relaxed)) return false;
    ...
    sampling_thread_started.store(true, std::memory_order_release);
    return true;
}
 ...
 *
 * If profiler-owned threads call this, we only request an asynchronous stop
 * and let a later non-profiler caller perform the full join/drain/reset.
 *
 */
void mach_sampling_profiler::stop_sampling() {
    if ((sampling_thread_started.load(std::memory_order_acquire) && pthread_equal(pthread_self(), sampling_thread))
    ...
    if (sampling_thread_started.load(std::memory_order_acquire)) {
        pthread_join(sampling_thread, nullptr);
        sampling_thread_mach.store(MACH_PORT_NULL, std::memory_order_relaxed);
        sampling_thread_started.store(false, std::memory_order_release);
    }
